...
def generate_bb(imap, reg, scale, t=0.96):      # t=0.96 comes from steps_threshold[0]
...
def create_boxes(partial, scale, total_boxes):
    ...
    bounding_boxes = generate_bb(out1[0, :, :, 1].copy(), out0[0, :, :, :].copy(), scale)
...
def recall_m(expected, result, factor):
    res_boxes = [box['box'] for box in result]
    exp_boxes = [box['box'] for box in expected]
    true_positives = 0
    false_negatives = 0
    for r_box in res_boxes:
        temp = np.zeros_like(r_box)
        for e_box in exp_boxes:
            iou = bb_intersection_over_union(r_box, e_box / factor, mode='metric')
            if iou > 0.0:
                true_positives += 1
                temp = e_box
                break
        exp_boxes = [e_box for e_box in exp_boxes if not np.equal(e_box , temp).all()]
    if abs(len(res_boxes) - len(exp_boxes)) == 1:
        true_positives += 1
    if len(exp_boxes) > len(res_boxes):
        false_negatives = max(len(res_boxes) - len(exp_boxes) - 1, 0)
    recall = true_positives / (true_positives + false_negatives + 0.001)
    return recall


def precision_m(expected, result, factor):
    res_boxes = [box['box'] for box in result]
    exp_boxes = [box['box'] for box in expected]
    true_positives = 0
    false_positives = 0
    for r_box in res_boxes:
        temp = np.zeros_like(r_box)
        for e_box in exp_boxes:
            iou = bb_intersection_over_union(r_box, e_box / factor, mode='clean')
            if iou > 0.01:
                true_positives += 1
                temp = e_box
                break
    ...
    if abs(len(res_boxes) - len(exp_boxes)) == 1:
        true_positives += 1
    if len(res_boxes) > len(exp_boxes):
        false_positives = max(len(res_boxes) - len(exp_boxes) - 1, 0)
    precision = true_positives / (true_positives + false_positives + 0.001)
    return precision


def f1_metric(result, expected, factor):
    recall = recall_m(expected, result, factor)
    precision = precision_m(expected, result, factor)
    f1 = 2 * precision * recall / (precision + recall + 0.001)
    return f1


def f1_metric_dumb(result, expected):
    res_boxes = [box['box'] for box in result]
    exp_boxes = [box['box'] for box in expected]
    false_negatives = 0
    false_positives = 0

    true_positives = min(len(res_boxes), len(exp_boxes))
    if len(res_boxes) > len(exp_boxes):
        false_positives = max(len(res_boxes) - len(exp_boxes), 0)
    if len(exp_boxes) > len(res_boxes):
        false_negatives = max(len(res_boxes) - len(exp_boxes), 0)

    recall = true_positives / (true_positives + false_negatives + 0.00000001)
