...
def main():
    ...
    for i in range(0, len(val_dict)):
        ...
        if image_ds.shape[1] > 70:
            continue
        image_ds = image_padding_max_ver(image_ds, max_size)
        image_to_detect = np.expand_dims(image_ds, axis=0)
        image_to_detect = (image_to_detect - 127.5) / 128.0
        complete_results = p_net.predict(image_to_detect)
        result = manage_result(complete_results)
